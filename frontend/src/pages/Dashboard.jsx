...
const HeroPill = memo(({ label, value }) => (
  <Box sx={{
    px: 2, py: 1.25, borderRadius: "10px",
    bgcolor: `color-mix(in srgb, ${C.navy} 8%, transparent)`, border: `1px solid ${C.navy}20`,
    textAlign: "center", minWidth: 100,
  }}>
    <Typography sx={{ fontSize: "16px", fontWeight: 800, color: C.navy, lineHeight: 1 }}>{value}</Typography>
...
/* ── Quick nav tile ── */
const NavTile = memo(({ label, count, icon, onClick }) => (
  <Paper
    elevation={0}
    onClick={onClick}
    sx={{
      ...
      "&:hover": {
        borderColor: C.navy,
        boxShadow: `0 8px 24px color-mix(in srgb, ${C.navy} 14%, transparent)`,
  ...
  >
    <Box sx={{
      ...
      bgcolor: `color-mix(in srgb, ${C.navy} 12%, transparent)`, flexShrink: 0,
      display: "flex", alignItems: "center", justifyContent: "center",
      "& svg": { fontSize: "20px !important", color: `${C.navy} !important` },
    }}>
      {icon}
    </Box>
    ...
    <ArrowForwardIcon className="tile-arrow" sx={{ color: C.navy, fontSize: 16, opacity: 0, transform: "translateX(-6px)", transition: "all 0.2s ease" }} />
  </Paper>
));

/* ── Metric box inside analytics panel ── */
const MetricBox = memo(({ label, value }) => (
  <Box sx={{ p: 2, borderRadius: "10px", bgcolor: `color-mix(in srgb, ${C.navy} 6%, transparent)`, border: `1px solid ${C.navy}15` }}>
    ...
    <Typography sx={{ fontSize: "1.1rem", fontWeight: 800, color: C.navy, letterSpacing: "-0.3px" }}>{value}</Typography>
  </Box>
));
...
const Dashboard = () => {
  ...
  const statCards = useMemo(() => [
    { title: "Total Products",    value: loading ? "—" : products.length,       icon: <Inventory2Icon />,           accentColor: C.blue,    bgColor: `color-mix(in srgb, ${C.blue} 15%, transparent)`,    trend: "In catalog",      trendUp: true  },
    { title: "Gross Revenue",     value: loading ? "—" : fmt(totalSelling),     icon: <TrendingUpIcon />,           accentColor: C.teal,    bgColor: `color-mix(in srgb, ${C.teal} 15%, transparent)`,    trend: "Selling value",   trendUp: true  },
    { title: "Purchase Cost",     value: loading ? "—" : fmt(totalPurchase),    icon: <AccountBalanceWalletIcon />, accentColor: C.amber,   bgColor: `color-mix(in srgb, ${C.amber} 15%, transparent)`,   trend: "Cost of goods"               },
    { title: "Gross Profit",      value: loading ? "—" : fmt(profit),           icon: <ShowChartIcon />,            accentColor: C.emerald, bgColor: `color-mix(in srgb, ${C.emerald} 15%, transparent)`, trend: `${margin}% margin`, trendUp: true },
  ], [loading, products.length, totalSelling, totalPurchase, profit, margin]);

  const navTiles = useMemo(() => [
    { label: "Products",  count: products.length,  icon: <Inventory2Icon />, path: "/products"  },
    { label: "Outlets",   count: outlets.length,   icon: <StoreIcon />,      path: "/outlets"   },
    { label: "Locations", count: locations.length, icon: <LocationOnIcon />, path: "/locations" },
    { label: "Divisions", count: divisions.length, icon: <CategoryIcon />,   path: "/divisions" },
    ...
    <Box sx={{ minHeight: "100vh" }}>
        ...
        <Box display="flex" alignItems={{ xs: "flex-start", sm: "center" }} justifyContent="space-between" flexDirection={{ xs: "column", sm: "row" }} gap={2} position="relative">
          <Box display="flex" alignItems="center" gap={2}>
            <Box sx={{
              ...
              <StoreMallDirectoryIcon sx={{ fontSize: 24, color: C.bg }} />
            </Box>
            <Box>
          ...
          <Box display="flex" gap={1} flexWrap="wrap">
            {loading ? [80,80,80].map((w,i) => <Skeleton key={i} variant="rounded" width={w} height={56} sx={{ borderRadius: "10px" }} />) : (
              <>
                <HeroPill label="Outlets"      value={outlets.length}  />
                <HeroPill label="Products"     value={products.length} />
                <HeroPill label="Margin"       value={`${margin}%`}   />
              </>
            )}
          </Box>
        </Box>
      ...
      {!loading && totalSelling > 0 && (
        <>
          ...
          <Paper elevation={0} sx={{ borderRadius: "14px", border: `1px solid ${C.border}`, overflow: "hidden", mb: 3.5, bgcolor: C.surface, boxShadow: "0 2px 8px rgba(15,23,42,0.04)" }}>
            {/* Panel header */}
            <Box sx={{ px: 2.5, py: 1.75, bgcolor: C.bgMuted, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <Box display="flex" alignItems="center" gap={1.25}>
                <Box sx={{ width: 28, height: 28, borderRadius: "7px", bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <SpeedIcon sx={{ fontSize: 15, color: C.navy }} />
                </Box>
                <Typography sx={{ fontWeight: 700, fontSize: "13.5px", color: C.navy }}>Profit Margin Overview</Typography>
              </Box>
              <Chip
                icon={<ArrowUpwardIcon sx={{ fontSize: "11px !important", color: `${C.navy} !important` }} />}
                label={`${margin}% Margin`}
                size="small"
                sx={{ bgcolor: `color-mix(in srgb, ${C.navy} 10%, transparent)`, color: C.navy, fontWeight: 700, fontSize: "11px", border: `1px solid ${C.navy}20`, height: 24 }}
              />
            </Box>

            <Box sx={{ p: 2.5 }}>
              <Grid container spacing={2} mb={2.5}>
                <Grid item xs={12} sm={4}><MetricBox label="Selling Revenue" value={fmt(totalSelling)} /></Grid>
                <Grid item xs={12} sm={4}><MetricBox label="Purchase Cost"   value={fmt(totalPurchase)} /></Grid>
                <Grid item xs={12} sm={4}><MetricBox label="Gross Profit"    value={fmt(profit)} /></Grid>
              </Grid>

              {/* Progress bar */}
              <Box display="flex" justifyContent="space-between" mb={0.75}>
                <Typography sx={{ fontSize: "11px", fontWeight: 600, color: C.slateMid }}>
                  Cost: <Box component="span" sx={{ color: C.navy }}>{costPct}%</Box>
                </Typography>
                <Typography sx={{ fontSize: "11px", fontWeight: 600, color: C.slateMid }}>
                  Profit: <Box component="span" sx={{ color: C.navy }}>{margin}%</Box>
                </Typography>
              </Box>
              <Box sx={{ height: 8, borderRadius: "6px", bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, overflow: "hidden" }}>
                <Box sx={{
              ...
              <Box display="flex" justifyContent="space-between" mt={0.75}>
                <Typography sx={{ fontSize: "10.5px", fontWeight: 600, color: C.navy, opacity: 0.8 }}>● Cost</Typography>
                <Typography sx={{ fontSize: "10.5px", fontWeight: 600, color: C.navy, opacity: 0.8 }}>● Profit</Typography>
              </Box>
            </Box>
          </Paper>
        </>
      ...
      <Grid container spacing={2.5}>
        ...
        <Grid item xs={12} md={6}>
          {loading ? <Skeleton variant="rounded" height={320} sx={{ borderRadius: "14px" }} /> : (
            <SummaryCard title="Recent Outlets" icon={<StoreIcon />} iconColor={C.navy} items={outlets} path="/outlets"
              renderItem={(o) => (
                <Box display="flex" alignItems="center" justifyContent="space-between" py={1.25} pl={2.5} pr={2}>
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ width: 32, height: 32, fontSize: "12px", fontWeight: 700, bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, color: C.navy, borderRadius: "8px" }}>
                  ...
                  <Chip label={o.outletType || "—"} size="small" sx={{ fontSize: "10px", fontWeight: 600, bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, color: C.navy, borderRadius: "5px", height: 20 }} />
                </Box>
              )}
            />
          )}
        ...
        <Grid item xs={12} md={6}>
          {loading ? <Skeleton variant="rounded" height={320} sx={{ borderRadius: "14px" }} /> : (
            <SummaryCard title="Recent Products" icon={<Inventory2Icon />} iconColor={C.navy} items={products} path="/products"
              renderItem={(p) => (
                <Box display="flex" alignItems="center" justifyContent="space-between" py={1.25} pl={2.5} pr={2}>
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ width: 32, height: 32, fontSize: "12px", fontWeight: 700, bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, color: C.navy, borderRadius: "8px" }}>
                  ...
                  <Box textAlign="right">
                    <Typography sx={{ fontSize: "12px", fontWeight: 700, color: C.navy, display: "block" }}>{fmt(p.sellingPrice || 0)}</Typography>
        ...
        <Grid item xs={12} md={6}>
          {loading ? <Skeleton variant="rounded" height={280} sx={{ borderRadius: "14px" }} /> : (
            <SummaryCard title="Locations" icon={<LocationOnIcon />} iconColor={C.navy} items={locations} path="/locations"
              renderItem={(l) => (
        ...
        <Grid item xs={12} md={6}>
          {loading ? <Skeleton variant="rounded" height={280} sx={{ borderRadius: "14px" }} /> : (
            <SummaryCard title="Divisions" icon={<CategoryIcon />} iconColor={C.navy} items={divisions} path="/divisions"
              renderItem={(d) => (
                <Box display="flex" alignItems="center" justifyContent="space-between" py={1.25} pl={2.5} pr={2}>
                  <Box display="flex" alignItems="center" gap={1.5}>
                    <Avatar sx={{ width: 32, height: 32, fontSize: "12px", fontWeight: 700, bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, color: C.navy, borderRadius: "8px" }}>
                  ...
                  {d.products?.length > 0 && (
                    <Chip label={`${d.products.length} products`} size="small" sx={{ fontSize: "10px", fontWeight: 600, bgcolor: `color-mix(in srgb, ${C.navy} 15%, transparent)`, color: C.navy, borderRadius: "5px", height: 20 }} />
                  )}
                </Box>
              )}
            />
