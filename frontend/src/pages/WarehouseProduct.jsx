...
import { GetWarehouseProducts, DeleteWarehouseProduct, UpdateWarehouseProduct, CreateWarehouseProduct } from "../services/WarehouseProductService";
...
export default function WarehouseProduct() {
  ...
  const { data, isLoading: loading } = useQuery({
    queryKey: ['warehouseProducts', page, filters],
    queryFn: async ({ signal }) => {
      ...
      const res = await GetWarehouseProducts(params, abortSignal);
  ...
  const mutation = useMutation({
    mutationFn: async (payload) => {
      if (selectedId) {
        return await UpdateWarehouseProduct(selectedId, payload);
      } else {
        return await CreateWarehouseProduct(payload);
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => DeleteWarehouseProduct(id),
    onSuccess: () => {
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: C.surface, minHeight: "100vh" }}>
      <PageHeader title="Warehouse Products" subtitle="Manage warehouse product catalog" onAdd={() => handleOpen()} addLabel="Add Product" />
      <ImportExportBar
        entity="Products"
