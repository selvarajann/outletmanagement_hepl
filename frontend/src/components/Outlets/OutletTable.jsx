...
export default function OutletTable({ outlets, onEdit, onDelete }) {
  ...
  const rows = outlets.map((o, i) => {
    ...
    return (
      ...
      >
        ...
        <TableCell>
          <Chip label={o.outletCode || "N/A"} size="small"
            sx={{ backgroundColor: C.slateLight, color: C.slate, fontWeight: 600, fontSize: 11, borderRadius: 1 }} />
        </TableCell>
        <TableCell>
          <Typography fontSize={13} color={C.slate}>{o.locationName || "—"}</Typography>
        </TableCell>
        <TableCell>
          <Chip label={o.outletType || "—"} size="small"
            sx={{ backgroundColor: tc.bg, color: tc.color, fontWeight: 700, fontSize: 11, borderRadius: 1 }} />
        </TableCell>
        <TableCell>
          <Typography fontSize={13} color={C.slate}>{o.ownerName}</Typography>
        </TableCell>
        <TableCell>
          <Typography fontSize={13} color={C.slate} sx={{ maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {o.address}
          </Typography>
        </TableCell>
        <TableCell>
          <Box display="flex" flexWrap="wrap" gap={0.5}>
            {o.divisions?.map((d) => (
              <Chip key={d.id} label={d.name} size="small"
                sx={{ backgroundColor: C.emeraldLight, color: C.emerald, fontWeight: 600, fontSize: 10, borderRadius: 1 }} />
            ))}
          </Box>
        </TableCell>
        <TableCell>
          ...
          <Tooltip title="Delete">
            <IconButton size="small" onClick={() => onDelete(o.id)}
