...
const statusColors = {
  PENDING_RECEIPT: { bg: C.amberLight, text: C.amber, border: C.amberMid },
  PROCESSING:      { bg: C.amberLight, text: C.amber, border: C.amberMid },
  RECEIVED:        { bg: C.emeraldLight, text: C.emerald, border: C.emeraldMid },
  DELIVERED:       { bg: C.emeraldLight, text: C.emerald, border: C.emeraldMid },
...
export default function BatchTable({ batches, onReceive, onCancel, onEditPrices, onView, onViewItems }) {
        ...
        {isPendingReceipt(row.status) && (
          <>
            <Tooltip title="Mark Received">
              <IconButton size="small" onClick={() => onReceive(row)} sx={{ color: C.emerald, "&:hover": { backgroundColor: C.emeraldLight } }}>
            ...
            <Tooltip title="Cancel Batch">
              <IconButton size="small" onClick={() => onCancel(row.id)} sx={{ color: C.red, "&:hover": { backgroundColor: C.redLight } }}>
  ...
  return <EnterpriseTable columns={columns} data={batches} emptyMessage="No batches found" />;
}
